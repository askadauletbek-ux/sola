            ...
            if now.hour == 14 and now.minute == 00:
                try:
                    ...
                    for u in users_to_update:
                        # 1. АТОМАРНОЕ ОБНОВЛЕНИЕ: Защита от гонки между 6 воркерами Gunicorn.
                        # Пытаемся изменить флаг напрямую в базе, не дожидаясь общего коммита.
                        updated_count = User.query.filter_by(id=u.id, new_user=True).update({'new_user': False})
                        db.session.commit()

                        # 2. ОТПРАВКА ПУША:
                        # Если updated_count > 0, значит именно этот воркер успел первым изменить базу.
                        # Другие воркеры получат 0 и не отправят дублирующие пуши.
                        if updated_count > 0:
                            try:
                                from notification_service import send_user_notification
                                send_user_notification(
                    ...
                    db.session.rollback()
                    print(f"DB Error in trial expiry block: {e}")
